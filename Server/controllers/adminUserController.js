...
const subadminCreate = async (req, res) => {
    ...
    try {
      ...
      const schema = joi.object({
            ...
            if (validate.error) {
              ...
              const newUser = await adminUserModel.createSubadmin( first_name,last_name,email,phone,hashedPassword,role );
  ...
  const updateAdminData = async (req, res) => {
    ...
    try {
      ...
      const result = await adminUserModel.updateAdminData( first_name, last_name, email,phone,hashedPassword,usid);
  
      return res.status(200).json({ message: "Admin updated successfully" ,Theresult:result.rows});
    } catch (error) {
      console.error(error);
  ...
  async function updatePicturee(req, res) {
    try {
      upload(req, res, async function (err) {
        ...
        const result = await db.query('UPDATE admin SET picture = $1 WHERE admin_id = $2 RETURNING admin_id, picture',
          [picture, usid]);
          console.log('Admin ID:', usid);

        if (!result.rows[0]) {
          return res.status(404).json({ success: false, error: 'Admin not found or picture update failed' });
        }
  
        const updatedImg = result.rows[0].picture;
  
        res.status(200).json({ success: true, message: 'picture updated successfully', updatedImg });
      });
    } catch (error) {
      console.error('An error occurred while updating the picture', error);
      res.status(500).json({ success: false, error: 'An error occurred while updating the picture' });
    }
  }
...
async function GetUsers(req, res) {
  try {
    ...
    const offset = (page - 1) * pageSize; ///فهرس البداية
    const checkQuery = `
    ...
    const result = await db.query(checkQuery, [pageSize, offset]);
...
async function getCoursesForUser(req, res) {
  try {
    ...
    const userCourses = await adminUserModel.getCoursesForUser(user_id,pageSize,offset);
...
async function addTasktoUser(req, res) {
  try {
    ...
    const result = await adminUserModel.addTasktoUser(user_id, task_id, start_date, end_date, admin_id, notes,course_id);
...
const SearchUsers = async (req, res) => {
  try {
    ...
    if (!email.trim()) {                   //trim >>>  عشان يتأكد اذا كان في فراغات او مسافات بالايميل عشان يتجاهلهم
...
async function checkTaskes(req, res) {
  try {
    const usid = req.user.user_id;
    const users_task_id = req.params.users_task_id;

    const result = await db.query(`UPDATE users_task SET show = true WHERE admin_id = $1 AND users_task_id = $2 AND show = 'false' RETURNING users_task_id`,
      [usid, users_task_id]);

    if (result.rows.length > 0) {
      const updatedTaskId = result.rows[0].users_task_id;
      res.status(200).json({ success: true, message: 'Task checked successfully', updatedTaskId });
    } else {
      res.status(400).json({ success: false, error: 'Task not found or already checked' });
    }
  } catch (error) {
    console.error('An error occurred while checking the task', error);
    res.status(500).json({ success: false, error: 'An error occurred while checking the task' });
  }
}
