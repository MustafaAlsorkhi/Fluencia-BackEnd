...
//     JOIN courses_user ON courses.course_id = courses_user.course_id
...
async function getStudentsInCourse(course_id) {
  try {
    const result = await db.query(`
      SELECT users.user_id, users.first_name, users.last_name ,users.email, users.picture
      FROM courses_user
      JOIN users ON courses_user.user_id = users.user_id
      WHERE courses_user.course_id = $1
    `, [course_id]);
